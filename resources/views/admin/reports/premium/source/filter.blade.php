...
<div class="end-bar">
    <div class="rightbar-title">
        ...
        <h5 class="m-0">Customer Wise Reports Filter</h5>
    </div>
    ...
    <div class="rightbar-content h-100" data-simplebar>
        <div class="container">
            <form action="{{ route('admin.reports.index') }}">
                    ...
                    <div class="col-sm-12">
                        <label for="source" class="col-form-label">Source Name</label>

                        <input type="hidden" name="type" value="source">

                        <select name="source" id="source" class="form-select select2">
                            <option value="" selected>Please Select Source</option>
                            @foreach($sources as $source)
                            <option value="{{ $source->lead_source }}"  {{ (request()->get('source') ==  $source->lead_source) ? 'selected' : '' }}>{{ $source->lead_source }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-12 mb-2 text-end" style="margin-top: 20px;">
                        ...
                        <a href="{{ route('admin.reports.index', ['type' => 'source']) }}" class="btn btn-sm btn-dark ms-1">Reset</a>
                    </div>
                </div>
            </form>
        </div>

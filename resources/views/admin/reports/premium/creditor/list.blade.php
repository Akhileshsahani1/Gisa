...
@section('title', 'Creditor Reports')
@section('head')
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">Creditor Reports</h4>
                </div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <select id="report_type_select"  class="form-control" name="report_type">
                                ...
                                <option value="customer" @if(request()->get('type') == 'customer') selected @endif>creditor Wise premium Report</option>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12 table-responsive">
                                <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                                    <thead class="bg-dark">
                                        <tr>
                                            ...
                                            <th class="bg-secondary text-white">{{ __('Statement count') }}</th>
                                            <th class="bg-secondary text-white">{{ __('Total') }}</th>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($statements as $statement)
                                            <tr>
                                                <td>{{ $i }}</td>
                                                <td class="table-user">
                                                    <a href="{{ route('admin.customers.show', $statement->customer_id) }}"
                                                        class="text-body fw-semibold">{{ $statement->firstname }}
                                                        {{ $statement->lastname }}</a>
                                                </td>
                                                <td>{{ $statement->total ??  '0'  }}</td>
                                                <td>{{ $statement->total_amt ? abs($statement->total_amt) :  '0' }}</td>                                                
                                            </tr>
                                            @php
                                            $i++;
                                            @endphp
                                ...
                                {{ $statements->appends(request()->query())->links('pagination::bootstrap-5') }}
                            </div>
                        </div>
                    </div>
                </div>
...
@endsection
@push('filter')
    @include('admin.reports.premium.creditor.filter')
@endpush
@push('scripts')

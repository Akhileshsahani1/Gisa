...
<div class="end-bar">
    <div class="rightbar-title">
        ...
        <h5 class="m-0">Quotation Policies Filter</h5>
    </div>

    <div class="rightbar-content h-100" data-simplebar>
        <div class="container">
            <form action="{{ route('admin.list.quotation-policies') }}">
                <div class="row mt-2">

                     <div class="col-sm-12">
                        <label class="col-form-label" for="policy_no">Policy No. </label>
                         <input type="text" class="form-control" id="policy_no" name="policy_no" placeholder="Enter Policy No." value="{{ $filter['policy_no'] }}">
                    </div>
                    ...
                    <div class="col-sm-12">
                        ...
                        <select name="customer_id" id="customer_id" class="form-select" >
                            ...
                            @foreach ($policies as $policy)
                                <option value="{{ isset($filter['customer_id']) ? $filter['customer_id'] : $policy->quotation->customer_id }}"
                                    {{ old('customer_id') ==  $policy->quotation->customer_id ? 'selected' : '' }}>
                                    {{ $policy->quotation->customer->firstname }}
                                    {{ $policy->quotation->customer->lastname }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-sm-12 mb-2 text-end">
                        ...
                        <a href="{{ route('admin.list.quotation-policies') }}" class="btn btn-sm btn-dark ms-1">Reset</a>
                    </div>
                </div>
            </form>
        </div>

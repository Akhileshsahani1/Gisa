        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">Convert Quotation to Policy</h4>
                </div>
            </div>
        </div>

        @include('admin.includes.flash-message')
        <form id="quotationForm" method="POST" action="{{ route('admin.quotation.convert') }}" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="card">
                        ...
                        <div class="card-body pt-1">
                            <div class="row">
                                    ...
                                    <select disabled name="policy_id" id="policy_id"
                                        ...
                                        @foreach ($policies as $policy)
                                            <option value="{{ $policy->id }}"
                                                {{ old('policy_id', $quotation->policy_id) == $policy->id ? 'selected' : '' }}>
                                                {{ $policy->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <input type="hidden" name="policy_id" value="{{ $quotation->policy_id }}">
                                <input type="hidden" name="policy_type_id" value="{{ $quotation->policy_type_id }}">
                                <input type="hidden" name="insurance_company" value="{{ motor_form($quotation->id,'selected_insurance_company') }}">
...
@endsection
@push('scripts')
  @include('admin.quotations.convert-policy-utilities')
@endpush

<div class="card">
    ...
    <div class="card-body pt-1">
        <div class="row">
            ...
            @else
            <input type="hidden" name="sales_executive_id" value="{{ Auth::user()?->id }}">
            <input type="hidden" name="service_executive_id" value="{{$service_executive_id}}">
            @endif
        </div>
    </div>
</div>
...
<div class="card opcard">
 <div class="row mt-3">
    <div class="form-group col-sm-12">
        <h4 for="terms" class="">Terms and conditions</h4>
        <textarea id="terms" name="terms" class="form-control">{{ isset($quotation) && motor_form($quotation->id, 'terms') ? motor_form($quotation->id, 'terms') : 'N/A'  }}</textarea>
    </div>
</div>
</div>

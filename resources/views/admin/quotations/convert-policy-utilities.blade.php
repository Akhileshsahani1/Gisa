...
<script>
    function getProductType() {
        ...
        $("#policy_type_id").html('');
        //$('#policy_id').val();
        var policy_id = '{{ $quotation->policy_id }}';
        var type_id = '{{ $quotation->policy_type_id }}';
...
<script>
    function getPolicyForm() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('admin.quotation.get-convert-form') }}',
            data: formData,
            dataType: 'json',
            beforeSend: function() {
                //
    ...
    jQuery(document).ready(function($) {
        ...
        $('body').delegate('#claim', 'change', function(e) {
            ...
            if (cval == 'Yes' || diffDays >= 90) {
                ...
                $('#ncb').attr('disabled', true).val('0%');
                $('form').append('<input type="hidden" id="dup_ncb" name="ncb" value="0%">');
            } else {

            ...
            <div class="card">
                <div class="card-body">
                    <form id="accountForm" method="POST"
                        action="{{ route('admin.user.update', $admin->id) }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group mb-2 {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select name="status" class="form-control">
                                <option value="1" @if($admin->status == '1') selected @endif>Enable</option>
                                <option value="0" @if($admin->status == '0') selected @endif>Disable</option>
                            </select>
                            @error('status')
                                <span id="name-error" class="error invalid-feedback">{{ $message }}</span>
                            @enderror

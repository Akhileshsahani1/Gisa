            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        @can('Create Policy')
                        <a href="{{ route('admin.policies.create') }}" class="btn btn-sm btn-primary float-end"><i
                                class="mdi mdi-plus"></i> Add Policy</a>
                        @endcan
                                            ...
                                            <tr>
                                                ...
                                                <td class="table-user">
                                                    <a href="{{ route('admin.policies.show', $policy) }}"
                                                        class="text-body fw-semibold">{{ $policy->name }}</a>
                                                </td>
                                                <td>
                                                    <span class="badge {{ $policy->enabled == true ? "bg-success" : "bg-danger" }}" disabled>{{ $policy->enabled == true ? "Enabled" : "Disabled" }}</span>
                                                </td>
                                                <td class="text-end">
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        @can('Show Policy')
                                                        <a href="{{ route('admin.policies.show', $policy) }}"
                                                            class="btn btn-sm btn-warning">Show Policy</a>
                                                        @endcan
                                                        @can('Delete Policy')
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $("#delete-all").click(function(e) {
            rows = [], $("input:checkbox[name=rows]:checked").each(function() {
                ...
                t.isConfirmed && ($("#delete-all").text("Deleting..."), e.preventDefault(), $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('admin.policies.bulk-delete') }}",
                    data: {
                        policies: rows,
                        _token: "{{ csrf_token() }}"
                    },
        ...
        $(".status").change(function() {
            var url = '{{ route("admin.policies.change-status", ":id") }}';
